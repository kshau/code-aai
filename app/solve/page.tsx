...
export default function Solve() {
	...
	const [codeFailedTestCase, setCodeFailedTestCase] = useState<ChallengeFailedTestCase | null>(null);
					...
					<Card className="p-6 flex flex-wrap">
						...
						<div className="flex flex-col pl-6 w-[15vw] gap-y-6">
							{codeFailedTestCase && (
								<Card className="h-full border-none mt-4">
									<CardHeader className="p-0 mb-4">
										...
										<CardDescription>
											Your code caused one or more test
											cases to fail! Details are below for
											one of them.
										</CardDescription>
									</CardHeader>
									<CardContent className="space-y-4 text-sm p-0">
										<div className="space-y-1">
											...
											<pre className="bg-neutral-800 rounded-sm p-2">
												{codeFailedTestCase.inputs.map((input, index) => (`${input.name} = ${input.value} ${(index > codeFailedTestCase.inputs.length + 1) ? "," : ""}`))}
											</pre>
										</div>
										<div className="space-y-1">
							...
							<Card className="h-full p-4">
								<pre
									className={`${
										codeProducedError && "text-red-600"
									} h-full`}
								>
									{codeOutput || (
